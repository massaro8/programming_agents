...
def test_profiles_have_exact_profile_shape(tmp_path: Path, profile: str) -> None:
    ...
    if profile != "minimal":
        expected |= {
            ...
            f"src/{package}/modules/__init__.py",
            f"src/{package}/modules/greeting/__init__.py",
            f"src/{package}/modules/greeting/domain/__init__.py",
            f"src/{package}/modules/greeting/domain/errors.py",
            f"src/{package}/modules/greeting/application/__init__.py",
            f"src/{package}/modules/greeting/application/service.py",
        }
    if profile == "service":
        expected |= {
            f"src/{package}/modules/greeting/adapters/__init__.py",
            f"src/{package}/modules/greeting/adapters/console.py",
        }
    assert files == expected
    assert tomllib.loads((target / ".agentready/manifest.toml").read_text())["profile"] == profile
    forbidden = {"application", "domain", "adapters"}
    if profile == "minimal":
        forbidden |= {"modules", "entrypoints"}
    assert not any((target / "src" / package / name).exists() for name in forbidden)
